...
export class CodeNarcCaller {
    ...
    async callCodeNarcServer(startServerTried = false) {
        ...
        try {
            ...
            // Do not catch body read/parse errors here: a connection reset or timeout while the body is
            // streaming must reach the catch block below, which owns the restart/retry/timeout handling
            ...
            if (!response.ok) {
                ...
                // Server-side unexpected error (HTTP 5xx): return status 2 so the caller falls back to direct java call
                ...
                // Invalid request (HTTP 4xx)
                return {
                    codeNarcStdErr: responseData?.errorDtl,
                    status: responseData?.statusCode ?? 2,
                    error: {
                        msg: `exception: ${responseData?.exceptionType} message: ${responseData?.errorMessage}`,
                        stack: new Error(`CodeNarcServer HTTP ${response.status}`).stack,
                        responseData: responseData?.errorDtl,
        ...
        if (responseData.status === "cancelledByDuplicateRequest") {
        ...
        if ([666, 1].includes(javaResult.status)) {
            if (!secondAttempt) {
                ...
                if (
                    ...
                    reason =
                        "Java is required to run npm-groovy-lint, as CodeNarc is written in Java/Groovy. Please verify that Java (version 17 minimum) is installed and available in your PATH: https://www.java.com/download";
                }
                return {
