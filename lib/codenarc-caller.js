...
export class CodeNarcCaller {
    ...
    getCodeNarcServerUri() {
        ...
        // The Java server only listens on the IPv4 loopback: normalize localhost so a host where it
        // resolves to ::1 only (or where network family autoselection is disabled) still connects
        const hostIpv4 = host.replace(/\/\/localhost\/?$/i, "//127.0.0.1");
        return hostIpv4 + ":" + (this.options.serverport || serverOptions.serverport);
    }
