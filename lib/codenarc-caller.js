...
export class CodeNarcCaller {
  ...
  constructor(codenarcArgs1, serverStatus1, args1, options1, execOpts1) {
    ...
    this.additionalJavaArgs = options1.javaoptions
      ? options1.javaoptions.split(",")
      : this.additionalJavaArgs;
  }
  ...
  async callCodeNarcServer(startServerTried = false) {
    ...
    const axiosConfig = {
      ...
      data: {
        ...
        parse:
          this.options.parse !== false && this.execOpts.onlyCodeNarc === false,
        fileList: this.execOpts.groovyFileName
          ? [this.execOpts.groovyFileName]
          : this.execOpts.inputFileList,
  ...
  async callCodeNarcJava(secondAttempt = false) {
    ...
    const javaCallerMode =
      secondAttempt === false ? "codeNarcServer" : "codeNarcJava";
    ...
    const javaResult = await javaCaller.run(scriptArgs, {
      detached: false,
    ...
    if ([666, 1].includes(javaResult.status)) {
      if (!secondAttempt) {
        ...
        trace(
          `Error calling CodeNarcServer via java: ${JSON.stringify(javaResult)}`,
        );
        ...
        if (
          javaResult.stderr &&
          (/node(.*)is not recognized as an internal or external command/gm.test(
            javaResult.stderr,
          ) ||
  ...
  async startCodeNarcServer() {
    ...
    if ([666, 1].includes(javaResult.status)) {
      console.error(
        c.red(`Unable to start CodeNarc server: ${JSON.stringify(javaResult)}`),
      );
    ...
    await new Promise((resolve) => {
      ...
      const onProcessExit = (code) => {
        if (!notified) {
          notified = true;
          debug(
            `CodeNarcServer process exited with code ${code} before becoming ready`,
          );
          this.serverStatus = "error";
          resolve();
        }
        ...
        axios
          ...
          .then((response) => {
            if (response.status === 200) {
              ...
              notified === false &&
              this.serverStatus === "unknown" &&
              performance.now() - start > maxAttemptTimeMs
            ) {
              ...
              this.declareServerError({
                message: `Timeout after ${since}ms} status: ${response.status}`,
              });
              resolve();
            } else {
              scheduleNext();
          ...
          .catch((e) => {
            ...
            if (
              notified === false &&
              this.serverStatus === "unknown" &&
              since > maxAttemptTimeMs
            ) {
              // Timeout has been reached
              debug(`Ping timeout after ${maxAttemptTimeMs}ms`);
              cleanup();
              this.declareServerError({
                message: `Timeout after ${since}ms error: ${e}`,
              });
              resolve();
            } else {
              scheduleNext();
  ...
  async killCodeNarcServer() {
    ...
    try {
      ...
      if (
        e.stack.includes("connResetException") ||
    ...
    await new Promise((resolve) => {
      interval = setInterval(() => {
        debug(
          `pinging CodeNarcServer at ${serverPingUri} serverStatus: ${this.serverStatus}`,
        );
        axios
          .get(serverPingUri)
          .then((response) => {
  ...
  getCodeNarcServerUri() {
    ...
    return (
      (this.options.serverhost || serverOptions.serverhost) +
      ":" +
      (this.options.serverport || serverOptions.serverport)
    );
  }
