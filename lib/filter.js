...
function collectDisabledBlocks(allLines) {
  ...
  for (const line of allLines) {
    ...
    if (
      line.includes("groovylint-disable") &&
      !line.includes("groovylint-disable-line") &&
      !line.includes("groovylint-disable-next-line")
    ) {
    ...
    // Disable end comment
      ...
      const matchingIndex = disabledBlocks.findIndex(
        (disabledBlock) =>
          ...
          disabledBlock.rules.sort().toString() ==
            enabledRules.sort().toString(),
      );
      if (matchingIndex > -1) {
        disabledBlocks[matchingIndex].endLine = lineNb;
      } else {
        console.warn(
          `npm-groovy-lint: Unable to find matching groovylint-disable for ${line}`,
        );
      }
    }
    lineNb++;
...
function isFilteredError(errItem, allLines, disabledBlocks) {
  ...
  const matchingDisabledBlocks = disabledBlocks.filter(
    (disabledBlock) =>
      disabledBlock.startLine <= linePos &&
      disabledBlock.endLine >= linePos &&
      checkMatch(disabledBlock.rules, ruleName),
  );
  if (matchingDisabledBlocks.length > 0) {
    return true;
  }
  // groovylint-disable-line on same line
  const lineConfigCurrLine = parseGroovyLintComment(
  ...
  const lineConfigNextLine = parseGroovyLintComment(
...
function checkMatch(lineConfig, ruleName) {
  return (
    lineConfig !== false &&
    (lineConfig[0] === "all" || lineConfig.includes(ruleName))
  );
}
