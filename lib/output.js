...
function buildSarifResult(lintResult) {
    ...
    sarifRunBuilder.initSimple({ name: "npm-groovy-lint", url: "https://nvuillam.github.io/npm-groovy-lint/" });
    for (const fileNm of Object.keys(lintResult.files)) {
        const fileErrors = lintResult.files[fileNm].errors;
        for (const err of fileErrors) {
            ...
            if (err.range) {
                (sarifResultInit.startLine = err.range.start.line),
                    (sarifResultInit.startColumn = err.range.start.column),
                    (sarifResultInit.endLine = err.range.end.line),
                    (sarifResultInit.endColumn = err.range.end.column);
            }
