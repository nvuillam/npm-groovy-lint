...
function colorsEnabledByEnv() {
    // The ansi-colors package this replaces colorized unless FORCE_COLOR=0, with no terminal detection;
    // that is kept deliberately, because the txt output is already switched off when it goes to a file and
    // the tests assert on ansi codes in piped output. NO_COLOR support is the one addition
    // (https://no-color.org: any non-empty value disables).
    return !process.env.NO_COLOR && process.env.FORCE_COLOR !== "0";
}
