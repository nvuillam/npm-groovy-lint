...
// Minimal replacement of the ansi-colors package: always colorize unless disabled by the standard
// NO_COLOR / FORCE_COLOR=0 env variables, or when output goes to a text file.
// Evaluated per call and not latched in a module variable, so a single run writing to a text file
// does not silently strip colors from the later runs of an in-process caller (VS Code extension, tests).
function colorsEnabledByEnv() {
    const env = process.env;
    // NO_COLOR disables colors when set to any non-empty value (https://no-color.org)
    if (env.NO_COLOR || env.FORCE_COLOR === "0" || env.TERM === "dumb") {
        return false;
    }
    // Same conditions as the ansi-colors package this replaces: colorize only when explicitly asked for,
    // on a Windows console, on an interactive terminal, or on a CI runner whose logs render ansi codes.
    // Without the TTY check, redirecting the output to a file would fill it with escape sequences.
    return env.FORCE_COLOR != null || process.platform === "win32" || process.stdout.isTTY === true || env.CI != null;
}
function applyColor(format, text, colorsEnabled) {
    // ansi-colors returned an empty string for a nullish or empty value: keep that, so a missing counter
    // does not end up printed as the literal "undefined"
    if (text == null || text === "") {
        return "";
    }
    return colorsEnabled ? styleText(format, String(text), { validateStream: false }) : String(text);
}
...
async function processOutput(outputType, output, lintResult, options, fixer = null) {
    ...
    if (outputType === "txt") {
        // Disable ansi colors if output results in a text file or if there is no output result
        const colorsEnabled = colorsEnabledByEnv() && !output.includes(".txt") && output !== "none";
        // Errors
        for (const fileNm of Object.keys(lintResult.files)) {
            const fileErrors = lintResult.files[fileNm].errors;
            let fileOutputString = applyColor("underline", fileNm, colorsEnabled) + "\n";
            ...
            for (const err of fileErrors) {
                ...
                fileOutputString +=
                    ...
                    "  " +
                    applyColor(color, err.severity.padEnd(7, " "), colorsEnabled) +
                    "  " +
                    err.msg +
                    "  " +
                    err.rule.padEnd(24, " ") +
        ...
        outputString += "\nnpm-groovy-lint results in " + applyColor("bold", lintResult.summary.totalFilesLinted, colorsEnabled) + " linted files:";
