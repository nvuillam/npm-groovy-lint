...
function appendErrorTypeStats(severity, fileErrors, counterResultsSummary) {
    // Found
    const fileFoundSeverityNumber = fileErrors.filter(err => err.severity === severity).length;
    // Fixed
    const fileFixedSeverityNumber = fileErrors.filter(err => {
        return err.severity === severity && err.fixed && err.fixed;
    }).length;
    // Remaining
...
function processOutput(outputType, output, lintResult, options, fixer = null) {
    let outputString = "";
