...
let colorsEnabled = process.env.NO_COLOR == null && process.env.FORCE_COLOR !== "0";
function applyColor(format, text) {
...
async function processOutput(outputType, output, lintResult, options, fixer = null) {
    ...
    if (outputType === "txt") {
        ...
        for (const fileNm of Object.keys(lintResult.files)) {
            ...
            let fileOutputString = applyColor("underline", fileNm) + "\n";
            ...
            for (const err of fileErrors) {
                ...
                fileOutputString +=
                    ...
                    "  " +
                    applyColor(color, err.severity.padEnd(7, " ")) +
                    "  " +
                    err.msg +
                    "  " +
                    err.rule.padEnd(24, " ") +
        ...
        outputString += "\nnpm-groovy-lint results in " + applyColor("bold", lintResult.summary.totalFilesLinted) + " linted files:";
