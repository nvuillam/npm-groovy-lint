...
function computeStats(lintResult) {
  ...
  for (const fileName of Object.keys(lintResult.files)) {
    ...
    const fileErrorStats = appendErrorTypeStats(
      "error",
      fileErrors,
      counterResultsSummary,
    );
    const fileWarningStats = appendErrorTypeStats(
      "warning",
      fileErrors,
      counterResultsSummary,
    );
    const fileInfoStats = appendErrorTypeStats(
      "info",
      fileErrors,
      counterResultsSummary,
    );
    ...
    counterResultsSummary.totalFoundNumber =
      counterResultsSummary.totalFoundNumber +
      fileErrorStats.found +
      fileWarningStats.found +
      fileInfoStats.found;
    counterResultsSummary.totalFixedNumber =
      counterResultsSummary.totalFixedNumber +
      fileErrorStats.fixed +
      fileWarningStats.fixed +
      fileInfoStats.fixed;
    counterResultsSummary.totalRemainingNumber =
      counterResultsSummary.totalRemainingNumber +
      fileErrorStats.remaining +
      fileWarningStats.remaining +
      fileInfoStats.remaining;
  }

  // Set summary
  lintResult.summary = Object.assign(
    lintResult.summary || {},
    counterResultsSummary,
  );
  return lintResult;
...
function appendErrorTypeStats(severity, fileErrors, counterResultsSummary) {
  ...
  // Remaining
  const fileRemainingSeverityNumber =
    fileFoundSeverityNumber - fileFixedSeverityNumber;

  // Add counters for each problem type
  for (const err of fileErrors) {
    if (err.severity === severity) {
      counterResultsSummary.detectedRules[err.rule] = counterResultsSummary
        .detectedRules[err.rule]
        ? counterResultsSummary.detectedRules[err.rule] + 1
        : 1;
      if (err.fixed && err.fixed === true) {
        counterResultsSummary.fixedRules[err.rule] = counterResultsSummary
          .fixedRules[err.rule]
  ...
  counterResultsSummary[totalFoundKey] =
    counterResultsSummary[totalFoundKey] + fileFoundSeverityNumber;
  counterResultsSummary[totalFixedKey] =
    counterResultsSummary[totalFixedKey] + fileFixedSeverityNumber;
  counterResultsSummary[totalRemainingKey] =
    counterResultsSummary[totalRemainingKey] + fileRemainingSeverityNumber;
  return {
...
async function processOutput(
  ...
  if (outputType === "txt") {
    ...
    outputString +=
      ...
      ansiColors.bold(lintResult.summary.totalFilesLinted) +
      " linted files:";
  ...
  for (const fileNm of Object.keys(lintResult.files)) {
    ...
    for (const err of fileErrors) {
      ...
      if (
        ...
        err.range &&
        err.range.start &&
        (err.range.start.line === 0 || err.range.start.line > 0)
      ) {
