...
class NpmGroovyLintFix {
    ...
    constructor(lintResult, optionsIn) {
        ...
        this.format = optionsIn.format === Boolean(this.format);
    ...
    async parseFixableErrors(errorIds) {
        for (const fileNm of Object.keys(this.updatedLintResult.files)) {
                ...
                if (
                    ...
                    const fixableError = {
                        ...
                        rule: Object.assign(
                            ...
                            this.options.rules?.[err.rule] && typeof this.options.rules[err.rule] === "object"
            ...
            this.fixableErrors[fileNm].sort((a, b) => a.rule.priority > b.rule.priority ? 1 : a.rule.priority < b.rule.priority ? -1 : 0);
        }
            ...
            Object.keys(this.fixableErrors).map(async fileNm => {
                ...
                for (let i = 0; i < this.fixableErrors[fileNm].length; i++) {
                    ...
                    if (fileFixableError.rule.scope === "file") {
                        const allLinesNew = this.tryApplyFixRule(allLines.slice(), lineNb, fileFixableError, fileNm).slice(); // copy result lines
    ...
    applyFixRule(newLine, lineNb, fixableError) {
        ...
        // Function defined in rule
            ...
            try {
                if (this.fixRules && this.fixRules[0] === "TriggerTestError") {
                    throw Error("ERROR: Trigger test error (on purpose)");
                }
    ...
    updateLintResult(fileNm, errId, errDataToSet, fixesSameErrorOnSameLine = false) {
        ...
        if (errIndex > -1) {
            ...
            if (fixesSameErrorOnSameLine && !errDataToSet.fixed) {
                const sameLineSameRuleFixedErrors = this.updatedLintResult.files[fileNm].errors.filter(err => {
                    return err.line === error.line && err.rule === error.rule && err.fixed && err.id !== error.id;
                });
        ...
        if (errDataToSet.fixed || error.fixed) {
    ...
    updateNextErrorsRanges(allLines, allLinesNew, lineNb, fileNm) {
        ...
        this.fixableErrors[fileNm] = this.fixableErrors[fileNm].map(fixableError => {
            ...
            if (fixableError.lineNb > lineNb) {
                fixableError.lineNb += lengthDiff;
