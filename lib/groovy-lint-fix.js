...
import {
  getNpmGroovyLintRules,
  getFormattingRulesToAlwaysRun,
} from "./groovy-lint-rules.js";
...
export class NpmGroovyLintFix {
  ...
  constructor(lintResult, optionsIn) {
    this.updatedLintResult = structuredClone(lintResult); // Clone object to not compare the initial one
    ...
    if (this.options.fixrulesexclude && this.options.fixrulesexclude !== "") {
      this.fixRulesExcluded = this.options.fixrulesexclude
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
    }
  ...
  async run(optns = { errorIds: null, propagate: false }) {
    ...
    this.npmGroovyLintRules = this.options.groovyLintRulesOverride
      ? await import(this.options.groovyLintRulesOverride)
      : npmGroovyLintRules;
    // Start progress bar
  ...
  async parseFixableErrors(errorIds) {
    for (const fileNm of Object.keys(this.updatedLintResult.files)) {
      ...
      for (const err of fileErrors) {
        ...
        const allowedByInclude =
          this.fixRules == null ||
          this.fixRules.includes(err.rule) ||
          this.fixRules[0] === "TriggerTestError";
        // Check if rule is excluded
        const excluded =
          this.fixRulesExcluded && this.fixRulesExcluded.includes(err.rule);
        ...
        if (
          ...
          const fixableError = {
            ...
            rule: Object.assign(
              ...
              this.options.rules &&
                this.options.rules[err.rule] &&
                typeof this.options.rules[err.rule] === "object"
          ...
          if (this.npmGroovyLintRules[err.rule].triggers) {
            for (const triggeredRuleName of this.npmGroovyLintRules[err.rule]
              .triggers) {
              // Skip if triggered rule is excluded
              if (
                this.fixRulesExcluded &&
                this.fixRulesExcluded.includes(triggeredRuleName)
              ) {
                continue;
              }
      ...
      if (this.format || this.fixRules == null) {
        ...
        for (const formattingRuleName of formattingRulesToAlwaysRun) {
          ...
          if (
            this.fixRulesExcluded &&
            this.fixRulesExcluded.includes(formattingRuleName)
          ) {
            continue;
          }
  ...
  addFixableError(fileNm, fixableError) {
    if (
      ...
      this.fixableErrors[fileNm].filter(
        (matchFixableError) =>
          matchFixableError.ruleName === fixableError.ruleName,
      ).length > 0
    ) {
      return;
    }
    ...
    await Promise.all(
      Object.keys(this.fixableErrors).map(async (fileNm) => {
        ...
        for (let i = 0; i < this.fixableErrors[fileNm].length; i++) {
          ...
          if (
            this.fixRulesExcluded &&
            this.fixRulesExcluded.includes(fileFixableError.ruleName)
          ) {
            continue;
          }
          if (
            this.fixRules != null &&
            !this.fixRules.includes(fileFixableError.ruleName) &&
            this.fixRules[0] !== "TriggerTestError"
          ) {
            continue;
          }
          const lineNb = fileFixableError.lineNb
            ? parseInt(fileFixableError.lineNb, 10) - 1
            : -1;
          let fixSuccess = false;
          // File scope violation
          if (fileFixableError.rule.scope === "file") {
            const allLinesNew = this.tryApplyFixRule(
              [...allLines],
              lineNb,
              fileFixableError,
              fileNm,
            ).slice(); // copy result lines
            if (JSON.stringify(allLinesNew) !== JSON.stringify(allLines)) {
              fixSuccess = true;
              this.updateNextErrorsRanges(
                allLines,
                allLinesNew,
                fileFixableError.lineNb,
                fileNm,
              );
              ...
              debug(
                `Skipping ${fileFixableError.ruleName} as no change in file ${fileNm}`,
              );
            }
          }
          // Line scope violation
            ...
            const fixedLine = this.tryApplyFixRule(
              line,
              lineNb,
              fileFixableError,
              fileNm,
            ...
            if (fixedLine !== line) {
              ...
              debug(
                `Skipping ${fileFixableError.ruleName} as no change in line: ${line}`,
              );
            }
          }
          // Update lint results
          const fixedNb = this.updateLintResult(
            ...
            fileFixableError.id,
            {
              ...
              triggersAgainAfterFix:
                fileFixableError.rule.triggersAgainAfterFix,
            },
  ...
  applyFixRule(line, lineNb, fixableError) {
    ...
    if (fix.type === "replaceString") {
      ...
      if (
        strBefore instanceof RegExp ||
        (!strBefore.includes("{{") && !strAfter.includes("{{"))
      ) {
        newLine = newLine.replace(strBefore, strAfter);
      } else {
        trace(
          `GroovyLint: missing replacement variable(s):\n${strBefore}\n${strAfter}\n${JSON.stringify(fixableError)}`,
        );
      }
    }
    // Function defined in rule
      ...
      try {
        ...
        trace(
          `ERROR: Fix function error: ${e.message} / ${JSON.stringify(fixableError)}`,
        );
        throw e;
      }
    } else {
      debug(
        `Fix type not supported: ${fix.type} / ${JSON.stringify(fixableError)}`,
      );
    }
    return newLine;
  }
  ...
  updateLintResult(
    ...
    const errIndex = this.updatedLintResult.files[fileNm].errors.findIndex(
      (error) => error.id === errId,
    );
    ...
    if (errIndex > -1) {
      ...
      if (fixesSameErrorOnSameLine && errDataToSet.fixed === false) {
        const sameLineSameRuleFixedErrors = this.updatedLintResult.files[
          fileNm
        ].errors.filter((err) => {
          return (
            err.line === error.line &&
  ...
  updateNextErrorsRanges(allLines, allLinesNew, lineNb, fileNm) {
    ...
    this.fixableErrors[fileNm] = this.fixableErrors[fileNm].map(
      (fixableError) => {
