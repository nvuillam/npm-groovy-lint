...
function listFileUsedFrameworks(file, source) {
    ...
    /* Grails TODO
    if() {
        frameworkKeys.push('grails');
    } */
    // Griffon
    if (source.includes("import griffon.")) {
        frameworkKeys.push("griffon");
    }
    // Micronaut
    if (source.includes("import io.micronaut")) {
        frameworkKeys.push("micronaut");
    }
    // Spring Boot
    if (source.includes("@SpringBootApplication")) {
        frameworkKeys.push("springboot");
    }
    // Ratpack
    if (source.includes("import ratpack.")) {
        frameworkKeys.push("ratpack");
    }
    /* Vert.x TODO
    if() {
        frameworkKeys.push('vertx');
    } */
    /* Kiss Web Framework TODO
    if() {
        frameworkKeys.push('kisswebframework');
    } */
    // Gradle
    if (file.includes(".gradle")) {
        frameworkKeys.push("gradle");
    }
    // Spock
    if (source.includes("import spock")) {
        frameworkKeys.push("spock");
    }
    // Gaelyk
    if (source.includes("import groovyx.gaelyk.")) {
        frameworkKeys.push("gaelyk");
    }
    // Jenkinsfile
    if (file.includes("Jenkinsfile")) {
        frameworkKeys.push("jenkinsfile");
    }
    // Jenkins Shared library
    if (source.includes("@NonCPS") || source.includes("Jenkins shared library")) {
        frameworkKeys.push("jenkins-shared-lib");
    }
    // GPars
    if (source.includes("import static groovyx.gpars") || source.includes("import groovyx.gpars")) {
        frameworkKeys.push("gpars");
    }
