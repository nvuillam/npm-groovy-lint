...
describe("Lint with API", () => {
    ...
    it("(API:file) should generate text console output", async () => {
        const linter = await new NpmGroovyLint(
            [process.execPath, "", "--path", '"lib/example"', "--files", "**/" + SAMPLE_FILE_SMALL, "--no-insight", "--verbose"],
            {
                verbose: true
            }
        ).run();
    ...
    it("(API:file) should run on a Jenkinsfile", async () => {
        const linter = await new NpmGroovyLint(
            [process.execPath, "", "--path", '"lib/example"', "-f", "**/Jenkinsfile", "--no-insight", "--verbose"],
            {
                verbose: true
            }
        ).run();
    ...
    it("(API:files) should ignore fake_node_modules pattern", async () => {
        ...
        assert(
            linter.outputString.includes(`npm-groovy-lint results in ${c.bold(6)} linted files`),
