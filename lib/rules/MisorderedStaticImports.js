...
function addInImportsList(importsLs, commentLine, importLine) {
  const commentImportObjs = importsLs.filter(
    (importObj) => importObj.commentLine === commentLine,
  );
  const commentImportObj =
    commentImportObjs && commentImportObjs[0]
      ? commentImportObjs[0]
      : { commentLine: commentLine, importLines: [] };
  ...
  if (commentImportObjs && commentImportObjs.length > 0) {
    importsLs = importsLs.map((importObj) =>
      importObj.commentLine === commentLine ? commentImportObj : importObj,
    );
  } else {
    ...
    func: (allLines) => {
      ...
      for (const line of allLines) {
        ...
        if (
          ...
          allLines[pos + 1].trim().startsWith("import")
        ) {
          ...
          if (line.trimStart().startsWith("import static")) {
            const commentKey =
              imports.normal.filter(
                (importObj) => importObj.commentLine === lastCommentFound,
              ).length === 0
                ? lastCommentFound
                : "";
          ...
          firstImportFoundPos =
            firstImportFoundPos || firstImportFoundPos === 0
              ? firstImportFoundPos
              : pos;
      ...
      const startPos = firstCommentFoundPos
        ? Math.min(firstCommentFoundPos, firstImportFoundPos)
        : firstImportFoundPos;
      allLines.splice(
        startPos,
        lastImportFoundPos - startPos + 1,
        ...allImportLines,
      );
      return allLines;
    },
