      ...
      for (let i = 0; i < allLines.length; i++) {
        ...
        if (["}", "})", "});"].includes(line.trim())) {
          ...
          while (
            ...
            matchingLineIndent == null
          ) {
            const prevLine = allLines[i - j];
            // Remove content inside quotes (handling escaped quotes) to avoid counting braces inside strings
            const lineWithoutQuotedStuff = prevLine.replace(
              /(["'])((?:\\.|(?!\1).)*?)\1/g,
              "",
            );
            const closingBraces = (lineWithoutQuotedStuff.match(/}/g) || [])
              .length;
            const openingBraces = (lineWithoutQuotedStuff.match(/\{/g) || [])
              .length;
          ...
          if (matchingLineIndent || matchingLineIndent === 0) {
            line = (" ".repeat(matchingLineIndent) + line.trimStart()).replace(
              /\t/g,
              "",
            );
          }
