...
const rule = {
  ...
  fix: {
    ...
    func: (allLines, variables) => {
      ...
      const className = getVariable(variables, "CLASSNAME", {
        mandatory: true,
      });
      if (allLines[lineNumber].includes(className)) {
        allLines.splice(lineNumber, 1);
      } else {
        const itemIndex = allLines.findIndex((line) =>
          line.includes(className),
        );
        throw new Error(
          `FIX ERROR: UnusedImport was expecting ${className} at position ${lineNumber} but it is at position ${itemIndex} `,
        );
      }
      return allLines;
    },
