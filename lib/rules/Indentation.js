  ...
  variables: [
    {
      name: "EXPECTED",
      regex:
        /The (.*) is at the incorrect indent level: Expected column (.*) but was (.*)/,
      regexPos: 2,
      type: "number",
    },
    {
      name: "EXPECTED_LIST",
      regex:
        /The (.*) is at the incorrect indent level: Expected one of columns (.*) but was (.*)/,
      regexPos: 2,
      type: "array",
    },
    {
      ...
      regex:
        /The (.*) is at the incorrect indent level: Depending on your chaining style, expected one of (.*) or one of (.*) columns, but was (.*)/,
      regexPos: 2,
      type: "array",
    },
    {
    ...
    func: (errLine, errItem, evaluatedVars) => {
      return {
        start: { line: errItem.line, character: 0 },
          ...
          line: errItem.line,
          character: getVariable(evaluatedVars, "FOUND") - 1,
        },
      };
    },
  },
  fix: {
    ...
    func: (line, evaluatedVars) => {
      let expected = parseInt(
        getVariable(evaluatedVars, "EXPECTED", {
          mandatory: false,
          line: line,
        }),
        10,
      ...
      if (expected == null || isNaN(expected)) {
        expected = parseInt(
          (getVariable(evaluatedVars, "EXPECTED_LIST", {
            mandatory: false,
            line: line,
          }) || [null])[0],
          10,
      ...
      if (expected == null || isNaN(expected)) {
        expected = parseInt(
          (getVariable(evaluatedVars, "EXPECTED_LIST_CHAINING", {
            mandatory: false,
            line: line,
          }) || [null])[0],
          10,
