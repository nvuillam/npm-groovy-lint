...
const rule = {
    ...
    variables: [
        {
            name: "EXPECTED",
            regex: /The (.*) on line (.*) in (.*) is at the incorrect indent level: Expected column (.*) but was (.*)/,
            regexPos: 4,
            type: "number"
        },
        {
            name: "EXPECTED_LIST",
            regex: /The (.*) on line (.*) in (.*) is at the incorrect indent level: Expected one of columns (.*) but was (.*)/,
            regexPos: 4,
            type: "array"
        },
        {
    ...
    fix: {
        ...
        func: (line, evaluatedVars) => {
            ...
            const expectedCol = expected ? expected : parseInt(getVariable(evaluatedVars, "EXPECTED_LIST", { mandatory: true, line: line })[0], 10);
