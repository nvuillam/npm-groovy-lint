...
class NpmGroovyLint {
    ...
    async preProcess() {
        ...
        if (this.parseOptions) {
            ...
            if (this.options.failon && this.options.loglevel && !isErrorInLogLevelScope(this.options.failon, this.options.loglevel)) {
                this.status = 2;
                this.error = {
    ...
    */
    async callCodeNarc() {
        ...
        const codeNarcCaller = new CodeNarcCaller(this.codenarcArgs, this.serverStatus, this.args, this.options, {
            groovyFileName: this.tmpGroovyFileName || null,
    ...
    async postProcess() {
        ...
        // process npm-groovy-lint options ( output, fix, formatting ...)
            ...
            if (this.options.fix || this.options.format) {
                ...
                if (checkIfFixAgainRequiredRes.runAgain && this.origin !== "fixAgainAfterFix") {
                ...
                if (this.fixer.fixedErrorsNumber > 0 && !this.options.nolintafter && this.origin !== "fixAgainAfterFix") {
                    await this.lintAgainAfterFix();
                }
            }
            // Output result
    ...
    checkIfFixAgainRequired() {
        ...
        for (const file of Object.keys(this.lintResult.files)) {
            for (const err of this.lintResult.files[file].errors) {
                if (err.fixed && err.triggersAgainAfterFix && err.triggersAgainAfterFix.length > 0) {
                    runAgain = true;
                    if (!runAgainOnFiles[file])
                        runAgainOnFiles[file] = { fixrules: [] };
    ...
    mergeResults(lintResAfterFix, lintResControl) {
        ...
        for (const afterFixResFileNm of Object.keys(lintResAfterFix.files)) {
            ...
            const fixedErrors = afterFixFileErrors.filter(err => err.fixed);
            const controlFileErrors =
                lintResControl.files?.[afterFixResFileNm] ? lintResControl.files[afterFixResFileNm].errors || [] : [];
    ...
    mergeFixAgainResults(lintResToUpdate, lintResAfterNewFix) {
        if (lintResToUpdate.files?.[0]) {
            if (Object.keys(lintResAfterNewFix.files).length > 0) {
                const [key] = Object.keys(lintResAfterNewFix.files);
        ...
        } else if (lintResAfterNewFix?.files) {
