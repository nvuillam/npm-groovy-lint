...
class NpmGroovyLint {
    ...
    async fixErrors(errorIds, optns = {}) {
        ...
        // Layout is fixed, not reported: drop what the borrowed format rules could not fix
        this.filterBorrowedFormatRuleErrors();
        ...
        // Recompute the return code from the filtered results: filterBorrowedFormatRuleErrors()
        // resets a status raised by lintAgainAfterFix(), and unlike the postProcess() path this
        // public entry point has no later manageReturnCode() call - without this, an API caller
        // (e.g. the VS Code extension) would read status 0 despite remaining real violations.
    ...
    async manageDeleteTmpFiles() {
        ...
        await fsPromises.rm(this.tmpGroovyFileName, { force: true });
        debug(`Removed temp file ${this.tmpGroovyFileName} as it is not longer used :)`);
        const tmpDir = path.dirname(this.tmpGroovyFileName);
        if (tmpDir.includes("codeNarcTmpDir_") && (await fsPromises.readdir(tmpDir)).length === 0) {
    ...
    // Adds the `format` preset rules to a lint ruleset so --fix can repair layout.
    // The `recommended` preset reports likely mistakes only, so it carries no layout rule at
    // all; --format owns them, and --fix borrows them here.
    // Rules already present in the lint config keep their configuration, so a rule the
    // user disabled stays disabled.
    // Only rules the fix engine can actually repair are borrowed: a borrowed rule with no
    // fixer would make --fix report violations it can never fix, and so exit 1 where a
    // plain lint of the same file exits 0. Borrowed rule names are remembered so whatever
    // they could not fix is dropped from the final report (see postProcess): layout is
    // fixed, not reported.
    async addFormattingRulesForFix(configProperties) {
        if (configProperties.codenarcRulesets || (this.options.rulesets && this.options.rulesetsoverridetype !== "appendConfig")) {
            // The caller supplied its own ruleset; do not second-guess it.
            return;
        }
        const formatProperties = await loadConfig(
        ...
        configProperties.rules = configProperties.rules || {};
        this.borrowedFormatRules = [];
        for (const [ruleName, ruleDef] of Object.entries(formatProperties.rules || {})) {
            if (configProperties.rules[ruleName] === undefined && npmGroovyLintRules[ruleName]?.fix) {
                configProperties.rules[ruleName] = ruleDef;
                this.borrowedFormatRules.push(ruleName);
            }
        }
    }

    // Drops the violations that borrowed format rules could not fix: those rules were
    // injected only so --fix can repair layout, and a plain lint with the same
    // configuration would not have reported them at all. Fixed errors are kept, so the
    // output still shows what --fix repaired.
    filterBorrowedFormatRuleErrors() {
        if (!this.borrowedFormatRules?.length || !this.lintResult?.files) {
            return;
        }
        const borrowed = new Set(this.borrowedFormatRules);
        for (const file of Object.keys(this.lintResult.files)) {
            const fileResult = this.lintResult.files[file];
            fileResult.errors = (fileResult.errors || []).filter((err) => err.fixed === true || !borrowed.has(err.rule));
        }
        if (this.status === 1) {
            // The status may have been raised by a borrowed rule's remaining violations
            // (lintAgainAfterFix propagates its own status): reset it and let
            // manageReturnCode() recompute it from the filtered results.
            this.status = 0;
        }
    }
    ...
    async preProcess() {
        ...
        if (this.parseOptions) {
            try {
                ...
                // --fix must keep repairing layout even though the `recommended` preset carries
                // no layout rule, so the formatting rules are merged on top of the lint ruleset.
                // Only rules the lint config does not mention are added, so a rule the user set
                // to "off" stays off.
                if (this.options.fix && !this.options.format) {
                    await this.addFormattingRulesForFix(configProperties);
                }
    ...
    */
    ...
    async postProcess() {
        ...
        // process npm-groovy-lint options ( output, fix, formatting ...)
            ...
            // A CodeNarc result npm-groovy-lint cannot read carries a status and an error message
            // but no files or summary: there is nothing to fix, count or format, and going on
            // would fail on the missing fields rather than report the error.
            if (this.lintResult.status === 2) {
                this.status = this.lintResult.status;
                await this.manageDeleteTmpFiles();
                return;
            }
    ...
    async lintAgainAfterFix() {
        ...
        const lintAgainOptions = structuredClone(this.options);
    ...
    async fixAgainAfterFix(filesAndRulesToProcess) {
        const fixAgainOptions = structuredClone(this.options);
        ...
        if (this.options.fixrulesexclude) {
            const excludedRules = this.options.fixrulesexclude
                .split(",")
                .map((s) => s.trim())
                .filter(Boolean);
        ...
        // A triggered rule keeps the configuration the lint ruleset gives it, and is run bare when
        // the lint ruleset does not mention it: `recommended` carries no layout rule, yet fixing
        // braces still has to re-run Indentation. A rule the user disabled stays disabled, and the
        // npm-groovy-lint-only rules (IndentationClosingBraces...) stay out of the CodeNarc
        // ruleset, which would reject them - they are fixed through fixrules below.
        const npmGroovyLintRules = await getNpmGroovyLintRules();
        const lintRules = this.options.rules || {};
        fixAgainOptions.rulesets = fixRules
            .filter((ruleName) => npmGroovyLintRules[ruleName]?.isCodeNarcRule !== false)
            .filter((ruleName) => !isRuleDisabled(lintRules[ruleName]))
            .map((ruleName) => (lintRules[ruleName] === undefined ? ruleName : `${ruleName}${JSON.stringify(lintRules[ruleName])}`))
            .join(",");
        if (fixAgainOptions.rulesets === "") {
            debug(`All triggered rules are disabled by the configuration, skipping fixAgainAfterFix`);
            return;
        }
...
// A rule definition from a .groovylintrc config that switches the rule off, in any of the
// accepted notations
function isRuleDisabled(ruleDefinition) {
    return (
        ruleDefinition === "off" ||
        (ruleDefinition != null &&
            typeof ruleDefinition === "object" &&
            (ruleDefinition.severity === "off" || ruleDefinition.enabled === false || ruleDefinition.disabled === true))
    );
}
