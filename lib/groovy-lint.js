...
class NpmGroovyLint {
    ...
    // The `recommended` preset deliberately excludes the whitespace rules, which cost
    // about 45% of a lint run; --format owns them, and --fix borrows them here.
    ...
    async preProcess() {
        ...
        if (this.parseOptions) {
            try {
                ...
                // --fix must keep repairing layout even though the `recommended` preset no
                // longer carries the whitespace rules (they cost ~45% of a lint run), so the
                // formatting rules are unioned on top of the lint ruleset. Only rules the lint
                // config does not mention are added, so a rule the user set to "off" stays off.
