...
import {
  prepareCodeNarcCall,
  parseCodeNarcResult,
} from "./codenarc-factory.js";
import {
  NPM_GROOVY_LINT_CONSTANTS,
  loadConfig,
  getConfigFileName,
} from "./config.js";
...
class NpmGroovyLint {
  ...
  async fixErrors(errorIds, optns = {}) {
    ...
    debug(
      `Fix errors for ${JSON.stringify(errorIds)} on existing NpmGroovyLint instance`,
    );
    ...
    this.outputString = await processOutput(
      this.outputType,
      this.output,
      this.lintResult,
      this.options,
  ...
  async manageDeleteTmpFiles() {
    ...
    debug(
      `Removed temp file ${this.tmpGroovyFileName} as it is not longer used :)`,
    );
    const tmpDir = path.dirname(this.tmpGroovyFileName);
    if (
      tmpDir.includes("codeNarcTmpDir_") &&
      (await fs.readdir(tmpDir)).length === 0
    ) {
  ...
  async getConfigFilePath(path) {
    return await getConfigFileName(
      path || this.options.path || this.options.config,
  ...
  async preProcess() {
    ...
    if (this.parseOptions) {
      try {
        ...
        if (this.options._ && this.options._[0] === "-") {
          ...
          this.options._ = [];
          this.options.sourcefilepath =
            this.options.sourcefilepath || process.cwd();
      ...
      if (this.options.failon && this.options.loglevel) {
        if (
          !isErrorInLogLevelScope(this.options.failon, this.options.loglevel)
        ) {
          this.status = 2;
          this.error = {
    ...
    if (
      ["initialCall", "index"].includes(this.origin) &&
      this.options.insight === true
    ) {
    ...
    if (this.options.killserver) {
      ...
      const codeNarcCaller = new CodeNarcCaller(
        this.codenarcArgs,
        this.serverStatus,
        this.args,
        this.options,
    ...
    if (this.options.version) {
      ...
      const codeNarcVersionLinter = await new NpmGroovyLint(
        [process.execPath, "", "--codenarcargs", "-version"],
        {},
      ).run();
      const codeNarcVersionLines = [
        (
          await getSourceLines(
            codeNarcVersionLinter.codeNarcStdOut ||
      ...
      versions.push(
        `- Groovy version ${NPM_GROOVY_LINT_CONSTANTS["GroovyVersion"]} (superlite)`,
      );
    ...
    if (this.options.help) {
      if (this.options._.length) {
        this.outputString = optionsDefinition.generateHelpForOption(
          this.options._[0],
        );
      } else {
  ...
  async postProcess() {
    ...
    if (this.status === 9) {
      console.info(
        `GroovyLint: Request cancelled by duplicate call on requestKey ${this.requestKey}`,
      );
    }
    // CodeNarc error
    else if (
      (this.codeNarcStdErr &&
        [null, "", undefined].includes(this.codeNarcStdOut)) ||
      this.status > 0
    ) {
      this.status = 2;
      console.error(
    ...
    // process npm-groovy-lint options ( output, fix, formatting ...)
      ...
      if (this.options.fix || this.options.format) {
        ...
        if (
          checkIfFixAgainRequiredRes.runAgain === true &&
          this.origin !== "fixAgainAfterFix"
        ) {
        ...
        if (
          this.fixer.fixedErrorsNumber > 0 &&
          this.options.nolintafter !== true &&
          this.origin !== "fixAgainAfterFix"
        ) {
          await this.lintAgainAfterFix();
        }
  ...
  checkIfFixAgainRequired() {
    ...
    for (const file of Object.keys(this.lintResult.files)) {
      for (const err of this.lintResult.files[file].errors) {
        if (
          err.fixed === true &&
          err.triggersAgainAfterFix &&
          err.triggersAgainAfterFix.length > 0
        ) {
          runAgain = true;
          runAgainOnFiles[file] = runAgainOnFiles[file]
            ? runAgainOnFiles[file]
            : { fixrules: [] };
  ...
  async lintAgainAfterFix() {
    ...
    trace(
      `Fix is done, lint again with options ${JSON.stringify(lintAgainOptions)}`,
    );
  ...
  async fixAgainAfterFix(filesAndRulesToProcess) {
    ...
    fixAgainOptions.rulesets = Object.keys(this.options.rules)
      ...
      .map(
        (ruleName) =>
          `${ruleName}${JSON.stringify(this.options.rules[ruleName])}`,
      )
      .join(",");
    ...
    // Process lint & fix
    debug(
      `Fix triggered rule requiring another lint & fix, do it again with options ${JSON.stringify(fixAgainOptions)}`,
    );
    ...
    this.lintResult = this.mergeFixAgainResults(
      this.lintResult,
      newLinter.lintResult,
    );
  }
  ...
  mergeResults(lintResAfterFix, lintResControl) {
    ...
    for (const afterFixResFileNm of Object.keys(lintResAfterFix.files)) {
      ...
      const afterFixFileErrors =
        lintResAfterFix.files[afterFixResFileNm].errors;
      const fixedErrors = afterFixFileErrors.filter(
        (err) => err.fixed === true,
      );
      const controlFileErrors =
        lintResControl.files && lintResControl.files[afterFixResFileNm]
          ? lintResControl.files[afterFixResFileNm].errors || []
          : [];
      ...
      if (lintResAfterFix.files[afterFixResFileNm].updatedSource) {
        mergedLintResults.files[afterFixResFileNm].updatedSource =
          lintResAfterFix.files[afterFixResFileNm].updatedSource;
      }
    }
  ...
  mergeFixAgainResults(lintResToUpdate, lintResAfterNewFix) {
    if (lintResToUpdate.files && lintResToUpdate.files[0]) {
      if (Object.keys(lintResAfterNewFix.files).length > 0) {
        ...
        if (updtSource) {
          lintResToUpdate.files[0] = Object.assign(lintResToUpdate.files[0], {
            updatedSource: updtSource,
          });
        }
      }
    } else if (lintResAfterNewFix && lintResAfterNewFix.files) {
      for (const afterNewFixResFileNm of Object.keys(
        lintResAfterNewFix.files,
      )) {
        // Set updatedSource in results in provided
        if (lintResAfterNewFix.files[afterNewFixResFileNm].updatedSource) {
          lintResToUpdate.files[afterNewFixResFileNm].updatedSource =
            lintResAfterNewFix.files[afterNewFixResFileNm].updatedSource;
        }
      }
    }
  ...
  manageReturnCode() {
    ...
    // Fail on info
      ...
      failureLevel === "info" &&
      (errorNb > 0 || warningNb > 0 || infoNb > 0)
    ) {
      if (!["json", "sarif", "stdout"].includes(this.outputType)) {
        console.error(
