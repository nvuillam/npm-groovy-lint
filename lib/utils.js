...
function addImport(allLineLs, classToImport) {
  ...
  if (
    allLineLs
      .map((line) => line.trim())
      .findIndex((line) => line.startsWith(`import ${classToImport}`)) > -1
  ) {
    return allLineLs;
  }
  ...
  const packageIndex = allLineLs.findIndex((line) =>
  ...
  if (rule.range) {
    if (rule.range.type === "function") {
      try {
        ...
        debug(
          "GroovyLint: Range function error: " +
            e.message +
            " / " +
            JSON.stringify(rule) +
            "\n" +
  ...
  for (const varDef of variableDefs || []) {
    ...
    if (varDef.regex) {
      ...
      if (regexRes && regexRes.length > 1) {
        ...
        const value = decodeHTMLEntities(regexRes[regexPos]);
        const varValue =
...
function findRangeBetweenStrings(allLines, errItem, strStart, strEnd) {
  ...
  while (
    (isStartFound === false || isEndFound === false) &&
    pos < allLines.length
  ) {
    if (
      !isStartFound &&
      allLines[pos].indexOf(strStart) > -1 &&
      notBetweenQuotesOrComment(allLines[pos], strStart)
    ) {
      range.start = {
        line: pos + 1,
        character: allLines[pos].indexOf(strStart),
      };
      isStartFound = true;
    }
    if (
...
function getOutOfBracesStrings(str, exclude = []) {
  ...
  return outOfBracesStrings.filter(
    (item) => !exclude.includes(item) && item !== "",
  );
}
...
function getStringRangeMultiline(allLines, str, errItem, levelKey) {
  ...
  while (isFound === false && pos < allLines.length) {
    if (
      levelKey &&
      allLines[pos].indexOf(levelKey) > -1 &&
      notBetweenQuotesOrComment(allLines[pos], levelKey)
    ) {
      level = level + 1;
    }
    if (
      !isFound &&
      allLines[pos].indexOf(str) > -1 &&
      notBetweenQuotesOrComment(allLines[pos], str)
    ) {
      if (level === 1) {
...
function getVariable(
  evaluatedVars,
  name,
  optns = { mandatory: true, decodeHtml: false, line: "" },
) {
  const matchingVars = evaluatedVars.filter(
    (evaluatedVar) => evaluatedVar.name === name,
  );
  if (matchingVars && matchingVars.length > 0) {
    return optns.decodeHtml
      ? decodeHTMLEntities(matchingVars[0].value)
      : matchingVars[0].value;
  } else if (optns.mandatory) {
    throw (
      new Error(
        "GroovyLint fix: missing mandatory variable " +
          name +
          " in " +
          JSON.stringify(evaluatedVars),
      ) +
      "for line :\n" +
      optns.line
    );
...
function moveOpeningBracket(allLines, variables) {
  ...
  const realPos = allLines[range.end.line - 1].includes("{")
    ? range.end.line
    : allLines[range.end.line].includes("{")
      ? range.end.line + 1
      : -1;
  if (realPos === -1) {
  ...
  const removedBracketLine = allLines[realPos - 1]
    .substring(allLines[realPos - 1].indexOf("{") + 1)
    .trimEnd();
...
function notBetweenQuotesOrComment(str, substr) {
  ...
  const res = singleQuotesMatch
    .concat(doubleQuotesMatch)
    .filter((match) => match.includes(substr));
  if (res.length > 0) {
    return false;
  }
...
function containsExceptInsideString(line, subString) {
  ...
  const lineWithoutQuotedStuff = line.replace(
    /(["'])(?:(?=(\\?))\2.)*?\1/g,
    "",
