...
function addImport(allLineLs, classToImport) {
    ...
    for (const [i, line] of allLineLs.entries()) {
...
function evaluateRange(errItem, rule, evaluatedVars, errLine, allLines) {
    let range;
    if (rule.range && rule.range.type === "function") {
        try {
...
function findRangeBetweenStrings(allLines, errItem, strStart, strEnd) {
    ...
    while ((!isStartFound || !isEndFound) && pos < allLines.length) {
...
function getStringRangeMultiline(allLines, str, errItem, levelKey) {
    ...
    let isFound = false;
    let level = 0;
    while (!isFound && pos < allLines.length) {
...
function getVariable(evaluatedVars, name, optns = { mandatory: true, decodeHtml: false, line: "" }) {
    ...
    if (matchingVars && matchingVars.length > 0) {
        ...
        throw Error("GroovyLint fix: missing mandatory variable " + name + " in " + JSON.stringify(evaluatedVars)) + "for line :\n" + optns.line;
    } else {
        return null;
    }
}
...
function moveOpeningBracket(allLines, variables) {
    ...
    if (realPos === -1) {
        throw Error("Unable to find opening bracket");
    }
    if (
        allLines[realPos - 1]
            .trim()
        ...
        throw Error("Fix not applied: probably a CodeNarc false positive");
    }
