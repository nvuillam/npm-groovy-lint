...
async function parseCodeNarcResult(options, codeNarcBaseDir, codeNarcJsonResult, tmpGroovyFileName, parseErrors) {
    ...
    const tmpGroovyFileNameReplace =
        tmpGroovyFileName?.includes(CODENARC_TMP_FILENAME_BASE) ? path.resolve(tmpGroovyFileName) : null;
    ...
    if (parseErrors && Object.keys(parseErrors).length > 0) {
        for (const fileNm1 of Object.keys(parseErrors)) {
            ...
            for (const parseError of fileParseErrors) {
                ...
                let msg =
                    parseError.cause?.message ? parseError.cause.message : `Unknown parsing error: ${JSON.stringify(parseError)}`;
                ...
                const errItemParse = {
                    id: errId,
                    line: parseError.cause?.line ? parseError.cause.line : 0,
                    rule: "NglParseError",
                    severity: "error",
                    msg: msg
                };
                // Add range if provided
                if (parseError.cause?.startColumn) {
                    errItemParse.range = {
    ...
    for (const packageInfo of codeNarcJsonResult.packages) {
        for (const fileInfo of packageInfo.files) {
            ...
            for (const violation of fileInfo.violations) {
                ...
                if (tmpGroovyFileNameReplace)
                    errItem.msg = errItem.msg.replace(tmpGroovyFileNameReplace, "");
    ...
    if (result.rules == null && options.returnrules) {
        ...
        for (const ruleDef of codeNarcJsonResult.rules) {
            ...
            if (matchRules?.[0]) {
                const [ruleCategory] = matchRules[0].split(".");
...
function buildRuleSets(options) {
    ...
    if ((ruleSetsDef.length === 0 || options.rulesetsoverridetype === "appendConfig") && options.rules) {
        for (const ruleName of Object.keys(options.rules)) {
            ...
            if (!(ruleDef === "off" || ruleDef.disabled || !ruleDef.enabled)) {
