...
async function manageCreateRuleSetFile(options) {
    ...
    if (ruleSetsDef && ruleSetsDef.length > 0) {
        ...
        const indentation = "    ";
        let ruleSetSource = `ruleset {\n\n${indentation}description 'Generated by npm-groovy-lint (https://github.com/nvuillam/npm-groovy-lint#readme)'\n\n`;
        for (const rule of ruleSetsDef) {
            if (!(npmGroovyLintRules[rule.ruleName] && npmGroovyLintRules[rule.ruleName].isCodeNarcRule === false)) {
                let ruleDeclaration = `${indentation}${rule.ruleName}`;
                const ruleParameters = stringifyWithoutPropQuotes(rule);
                if (ruleParameters.length > 0) {
                    ruleDeclaration += " {\n"
                        + ruleParameters
                            .split(",")
                            .map(item => `${indentation}${indentation}${item}`)
                            .join(`\n`)
                        + `\n${indentation}}`;
                }
