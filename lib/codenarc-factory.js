            ...
            .map(pathname => {
                ...
                if (directoryExists(resolvedPath)) {
                    finalPattern = path
                        ...
                        .replace(/\\/gu, "/") + filePatterns
                }
                // Absolute path file
                if (fs.existsSync(finalPattern)) {
                    fileList.push(finalPattern);
                }
                // Relative file (1)
                else if (fs.existsSync(path.join(result.codeNarcBaseDir, finalPattern))) {
                    const absoluteFile = path.join(result.codeNarcBaseDir, finalPattern).replace(/\\/gu, "/");
                    fileList.push(absoluteFile);
                    finalPattern = absoluteFile;
                }
                // Relative file (2)
                else if (fs.existsSync(path.join(process.cwd(), finalPattern))) {
                    const absoluteFile = path.join(process.cwd(), finalPattern).replace(/\\/gu, "/");
                    fileList.push(absoluteFile);
                    finalPattern = absoluteFile;
                }
