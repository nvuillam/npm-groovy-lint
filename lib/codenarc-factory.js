...
async function prepareCodeNarcCall(options) {
  ...
  for (const [key, value] of Object.entries(options)) {
    if (typeof value === "string" && value.match(/^['"]/)) {
      throw new Error(
        `CodeNarc doesn't supported options found ${key} = ${value}`,
      );
    }
  }
  ...
  if (options.source) {
    ...
    // Use default random file name
      ...
      const tmpFileNm = CODENARC_TMP_FILENAME_BASE + randomUUID() + ".groovy";
      result.tmpGroovyFileName = path.resolve(cnPath + "/" + tmpFileNm);
      cnFiles = ["**/" + tmpFileNm];
    }

    await fs
      .writeFile(result.tmpGroovyFileName, normalizeNewLines(options.source))
      .catch((err) => {
        throw new Error(`Unable to write temp file: ${err}`); // Ensure we have a stack trace.
      });

    debug(
      `CREATE GROOVY temp file ${result.tmpGroovyFileName} with input source, as CodeNarc requires physical files`,
    );
  }

  // Define base directory
  const baseBefore =
    (cnPath !== "." && cnPath.startsWith("/")) ||
    cnPath.includes(":/") ||
    cnPath.includes(":\\")
      ? ""
      : process.cwd() + "/";
  ...
  if (options.rulesets && options.rulesets.startsWith("{")) {
    // JSON format
    result.codenarcArgs.push(
      `-ruleset=${encodeURIComponent(options.rulesets)}`,
    );
  } else {
    // File list format
    const rulesetFileArgs = options.rulesets.startsWith("file:")
      ? options.rulesets
      : `file:${options.rulesets}`;
  ...
  if (positionalArgs.length > 0) {
    ...
    positionalArgs.filter(Boolean).forEach((pathname) => {
      ...
      if (directoryExists(absolutePath)) {
        ...
        includes.push(
          ...filePatterns.map((filePattern) => patternBase + filePattern),
        );
  ...
  // Matching files pattern(s)
    ...
    const filePatterns = defaultFilesPattern.map(
  ...
  if (
    ...
    result.outputType = result.output.split(".").pop().endsWith("html")
      ? "html"
      : result.output.split(".").pop().endsWith("xml")
        ? "xml"
        : "";
...
async function getCodeNarcBaseDirFromFiles(positionalArgs) {
  ...
  if (
    !positionalArgs.every(
      (fileOrDirOrPattern) =>
        fs.existsSync(fileOrDirOrPattern) ||
        directoryExists(fileOrDirOrPattern),
    )
  ) {
    return process.cwd();
  }
...
async function parseCodeNarcResult(
  options,
  codeNarcBaseDir,
  codeNarcJsonResult,
  tmpGroovyFileName,
  ...
  if (
    !codeNarcJsonResult ||
    !codeNarcJsonResult.codeNarc ||
    !codeNarcJsonResult.packages
  ) {
  ...
  result.summary.totalFilesWithErrorsNumber = parseInt(
    pckgSummary.filesWithViolations,
    10,
  );
  ...
  const tmpGroovyFileNameReplace =
    tmpGroovyFileName && tmpGroovyFileName.includes(CODENARC_TMP_FILENAME_BASE)
      ? path.resolve(tmpGroovyFileName)
      : null;
    ...
    for (const fileNm1 of Object.keys(parseErrors)) {
      ...
      for (const parseMessage of fileParseErrors) {
        ...
        const errItemParse = {
          id: errId,
          line:
            parseError.cause && parseError.cause.line
              ? parseError.cause.line
              : 0,
          rule: "NglParseError",
          severity: "error",
  ...
  for (const packageInfo of codeNarcJsonResult.packages) {
    for (const fileInfo of packageInfo.files) {
      ...
      for (const violation of fileInfo.violations) {
        ...
        errItem.msg = tmpGroovyFileNameReplace
          ? errItem.msg.replace(tmpGroovyFileNameReplace, "")
          : errItem.msg;
        ...
        if (
          ...
          (options.loglevel === "warning" &&
            ["error", "warning"].includes(errItem.severity))
        ) {
          ...
          if (errRule) {
            ...
            if (errRule.range) {
              const evaluatedVars = evaluateVariables(
                errRule.variables,
                errItem.msg,
  ...
  if (
    result.rules == null &&
    (options.returnrules === true || options.output.includes("sarif"))
  ) {
    const configAllFileName = await getConfigFileName(__dirname, null, [
      ".groovylintrc-all.json",
    ]);
    const grooylintrcAllRules = Object.keys(
      JSON.parse(fs.readFileSync(configAllFileName, "utf8").toString()).rules,
    );
    const rules = {};
    for (const ruleDef of codeNarcJsonResult.rules) {
      ...
      const matchRules = grooylintrcAllRules.filter(
        (ruleNameX) => ruleNameX.split(".")[1] === ruleName,
      );
...
async function buildRuleSets(options) {
  ...
  if (
    options.rulesets &&
    (options.rulesets.endsWith(".groovy") || options.rulesets.endsWith(".xml"))
  ) {
  ...
  if (
    ...
    ruleSetsDef = ruleList.map((ruleFromArgument) => {
      ...
      if (ruleFromArgument.includes("{")) {
        ...
        const ruleOptionsJson = ruleFromArgument.substring(
          ruleFromArgument.indexOf("{"),
        );
  ...
  if (
    (ruleSetsDef.length === 0 ||
      options.rulesetsoverridetype === "appendConfig") &&
    options.rules
  ) {
    for (const ruleName of Object.keys(options.rules)) {
      ...
      const ruleFromRuleSetsArgPos = ruleSetsDef.findIndex(
        (ruleDef) => ruleDef.ruleName === ruleName,
      );
  ...
  ruleSetsDef = ruleSetsDef.sort((a, b) =>
...
function buildCodeNarcRule(ruleName, ruleFromConfig) {
  const ruleNameShort = ruleName.includes(".")
    ? ruleName.split(".")[1]
    : ruleName;
...
function getCodeNarcPriorityCode(ruleFromConfig) {
  if (
    ["error", "err"].includes(ruleFromConfig) ||
    ["error", "err"].includes(ruleFromConfig.severity)
  ) {
    return 1;
  } else if (
    ["warning", "warn"].includes(ruleFromConfig) ||
    ["warning", "warn"].includes(ruleFromConfig.severity)
  ) {
    return 2;
  } else if (
    ["info", "audi"].includes(ruleFromConfig) ||
    ["info", "audi"].includes(ruleFromConfig.severity)
  ) {
    return 3;
  }
  return null;
