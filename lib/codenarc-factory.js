...
async function prepareCodeNarcCall(options) {
    ...
    let cnFiles = parseFilePatterns(options.files);
    ...
    if (options.source) {
        ...
        await fsPromises.mkdir(cnPath, { recursive: true, mode: 0o700 }); // await is needed, ignore editor warning.
        ...
        await fsPromises.writeFile(result.tmpGroovyFileName, normalizeNewLines(options.source)).catch((err) => {
...
/**
 * Normalizes the --files option (a comma-separated list of Ant patterns) into the
 * array form the rest of prepareCodeNarcCall works with.
 *
 * @param {string|string[]} files Value of the files option
 * @return {string[]|undefined} Ant patterns, or undefined when none were provided
 */
function parseFilePatterns(files) {
    if (!files) {
        return undefined;
    }
    const patterns = (Array.isArray(files) ? files : String(files).split(",")).map((pattern) => pattern.trim()).filter(Boolean);
...
// Return the common parent directory of a list of absolute directory paths (replaces the commondir package)
function commonDir(folders) {
    const commonParts = folders.slice(1).reduce((common, folder) => {
        const parts = folder.split(path.sep);
        let i = 0;
        while (i < common.length && common[i] === parts[i]) {
            i++;
        }
        return common.slice(0, i);
    }, folders[0].split(path.sep));
    const common = commonParts.join(path.sep);
    // On Windows a lone drive letter ("C:") is drive-relative: append the separator to make it absolute
...
async function buildRuleSets(options) {
    ...
    if ((ruleSetsDef.length === 0 || options.rulesetsoverridetype === "appendConfig") && options.rules) {
        for (const ruleName of Object.keys(options.rules)) {
            ...
            // Add in the list of rules to test , except if it is disabled (same notations as
            // isRuleDisabled in groovy-lint.js: "off" string, severity "off", disabled, enabled:false)
            if (!(ruleDef === "off" || ruleDef.severity === "off" || ruleDef.disabled === true || ruleDef.enabled === false)) {
