...
async function importFresh(filePath) {
    ...
    if (isEsModuleFile(resolvedPath)) {
        // ES module config file: bypass the ESM cache with a unique query string. Node has no way to evict
        // from the ES module registry, so each reload leaves a dead record behind; that is accepted rather
        // than keying the query string on the file mtime, which would silently serve a stale config when two
        // edits land in the same millisecond. Only ES module config files in a long-lived host are affected.
        return await import(`${pathToFileURL(resolvedPath).href}?fresh=${Date.now()}-${importFreshCounter++}`);
    }
    clearRequireCacheTree(requireCjs.resolve(resolvedPath));
    return { default: requireCjs(resolvedPath) };
}
