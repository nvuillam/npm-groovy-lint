...
const configFormatFilenames = [
  ".groovylintrc-format.json",
  ".groovylintrc-format.js",
];
...
async function loadConfig(
  ...
  let configFilePath;
  if (
    configExtensions.includes(startPathOrFile.split(".").pop()) &&
    mode !== "format"
  ) {
    // Sent file name
    ...
    fileNames = configExtensions.map(
      (ext) => `.groovylintrc-${startPathOrFile}.${ext}`,
    );
    configFilePath = await getConfigFileName(
      sourcefilepath || process.cwd(),
      sourcefilepath,
      fileNames,
      "",
    );
  ...
  if (configFilePath && configUser.codenarcRulesets) {
    ...
    configUser.rulesets = configUser.codenarcRulesets
      .split(",")
      .map((rulesetFile) =>
        path.resolve(path.dirname(configFilePath) + "/" + rulesetFile),
      )
      .join(",");
  }
  ...
  if (mode === "format") {
    const customUserConfig = await loadConfig(
      startPathOrFile,
      "lint",
      sourcefilepath,
      fileNamesIn,
...
async function manageExtends(configUser) {
  if (configUser.extends) {
    const baseConfigFilePath = await findConfigInPath(__dirname, [
      `.groovylintrc-${configUser.extends}.json`,
    ]);
...
async function getConfigFileName(
  ...
  sourcefilepath,
  fileNames = configLintFilenames,
  defaultConfig = defaultConfigLintFileName,
) {
  ...
  if (configFilePath == null) {
    try {
      ...
      const dir = stat.isDirectory
        ? startPathOrFile
        : path.parse(startPathOrFile).dir;
      configFilePath = await findConfigInPath(dir, fileNames);
    } catch (e) {
      debug(`Unable to find config file for ${sourcefilepath} (${e.message})`);
    }
...
async function loadConfigFromFile(filePath) {
  ...
  if (
    configLoaded != null &&
    !filePath.includes(defaultConfigLintFileName) &&
    !filePath.includes(allConfigLintFileName)
  ) {
...
async function loadJSConfigFile(filePath) {
  try {
    ...
    const config =
      moduleLoaded &&
      typeof moduleLoaded === "object" &&
      "default" in moduleLoaded
...
async function loadPackageJSONConfigFile(filePath) {
  try {
    ...
    if (!Object.hasOwnProperty.call(packageData, "groovylintConfig")) {
      throw Object.assign(
        new Error(`${filePath} doesn't have 'groovylintConfig' property`),
        { code: "GROOVYLINT_CONFIG_FIELD_NOT_FOUND" },
      );
    }
