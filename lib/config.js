...
// Import a module bypassing the ESM cache, so config file updates are taken into account (replaces the import-fresh package)
async function importFresh(filePath) {
    return await import(`${pathToFileURL(path.resolve(filePath)).href}?fresh=${Date.now()}`);
}
