...
// The unique query string bypasses the ESM loader cache; CJS config files are additionally cached
// by the require cache (which ignores query strings), so that entry is cleared first.
let importFreshCounter = 0;
async function importFresh(filePath) {
    const resolvedPath = path.resolve(filePath);
    try {
        delete requireCjs.cache[requireCjs.resolve(resolvedPath)];
    } catch {
        // Not resolvable as CJS: nothing to clear
    }
