...
async function importFresh(filePath) {
    ...
    try {
        ...
        moduleLoaded = requireCjs(resolvedPath);
    } catch {
        // Not loadable as CJS (ES module syntax): handled below
    }
    if (moduleLoaded != null && moduleLoaded[Symbol.toStringTag] !== "Module") {
        return { default: moduleLoaded };
    }
    // ES module config file (or require(esm) returned the cached ESM namespace): bypass the ESM cache with a unique query string
