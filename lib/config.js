...
async function importFresh(filePath) {
    ...
    if (moduleLoaded != null && moduleLoaded[Symbol.toStringTag] === "Module") {
        ...
        // Return the namespace as the import() branch does, so the caller picks the default export when
        // there is one and falls back to the named exports when there is not
        return moduleLoaded;
    }
...
// `extends` accepts a single base config name or a list of them, so the framework add-on presets
// can be composed: { "extends": ["recommended", "grails", "tests"] }. Bases are merged from left
// to right (a later base wins over an earlier one), then the config's own rules win over them all.
async function manageExtends(configUser) {
    if (configUser.extends) {
        const baseConfigNames = Array.isArray(configUser.extends) ? configUser.extends : [configUser.extends];
        let baseRules = {};
        for (const baseConfigName of baseConfigNames) {
            const baseConfigFilePath = await findConfigInPath(__dirname, [`.groovylintrc-${baseConfigName}.json`]);
            if (baseConfigFilePath == null) {
                throw new Error(`Unable to find base configuration "${baseConfigName}" referenced in extends`);
            }
            ...
            baseRules = overrideRules(baseRules, baseConfig.rules);
        }
        configUser.rules = overrideRules(baseRules, configUser.rules);
        delete configUser.extends;
    }
    return configUser;
}

// Merge overriding rules into base rules. Overridden entries are deleted first so the overriding
// definition also takes the overriding config's position in the resulting rule list.
function overrideRules(baseRules, overridingRules) {
    for (const ruleName of Object.keys(overridingRules)) {
        delete baseRules[ruleName];
    }
    return Object.assign(baseRules, overridingRules);
}
...
async function loadConfigFromFile(filePath) {
    ...
    // Only a USER config file may set overriddenRules (the rules the user explicitly
    // configured, exposed to callers like the VS Code extension). Built-in presets all live
    // in this package's lib directory as .groovylintrc-<preset>.json: loading one - directly
    // (-c advanced) or as an extends base - must not overwrite the user's overrides with the
    // preset's full rule map.
    const isBuiltinPreset = path.dirname(path.resolve(filePath)) === __dirname && path.basename(filePath).startsWith(".groovylintrc-");
    if (configLoaded != null && !isBuiltinPreset) {
