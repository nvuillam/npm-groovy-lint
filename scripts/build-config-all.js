...
const CODENARC_RESOURCES_BASE_URL =
  "https://raw.githubusercontent.com/CodeNarc/CodeNarc/master/src/main/resources";
...
(async () => {
  ...
  const { rulesConfig: allRulesConfig, metadata: ruleMetadata } =
    buildAllRules(allLines);
  ...
  await buildGroovyLintJsonSchema(
    ruleMetadata,
    npmDefinedRules,
    rulePropertyMetadata,
  );
  const fixableRules = [];
...
async function buildGroovyLintJsonSchema(
  ruleMetadata,
  npmDefinedRules,
  rulePropertyMetadata,
) {
  ...
  const addRuleProperty = (name, description) => {
    ...
    rulesProperties[name] = buildRuleSchema(
      description,
      name,
      rulePropertyMetadata,
      severityEnum,
...
async function getAvailableBaseConfigs() {
  ...
  return libEntries
    .filter(
      (entry) => entry.startsWith(".groovylintrc-") && entry.endsWith(".json"),
    )
...
async function collectRulePropertyMetadata() {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "groovylint-schema-"),
  );
  ...
  try {
    stdout = execFileSync(
      javaExecutable,
      ["-cp", jarsClasspath, "groovy.ui.GroovyMain", scriptPath, resource.path],
      {
        encoding: "utf8",
...
async function downloadCodeNarcResource(filename) {
  ...
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "groovylint-codenarc-"),
  );
