...
**Custom flavor exception**: if the CI workflow references a [custom flavor](https://megalinter.io/latest/custom-flavors/) (an action or image named like `<owner>/megalinter-custom-flavor-<name>`), `MEGALINTER_FLAVOR`/`MEGALINTER_VERSION` do not model it — resolve the image from the CI workflow (or the custom flavor repository's `action.yml`, usually `ghcr.io/<owner>/<repo>/megalinter-custom-flavor:<tag>`) and pass it explicitly with `--image`.

```bash
npx mega-linter-runner                            # docker
npx mega-linter-runner --container-engine podman  # podman
npx mega-linter-runner --image ghcr.io/<owner>/<repo>/megalinter-custom-flavor:latest  # custom flavor
```
...
- **After a `--fix` run, audit what the fixers changed** (`git status --short` / `git diff --stat`, excluding `megalinter-reports/`). If a formatter rewrote many files in a style that contradicts the repository's own conventions — typical cause: the style is pinned in npm scripts or CI flags but the formatter has no config file to read (e.g. no `.prettierrc`), so it applied its defaults — **revert that churn** (`git restore`), report the mismatch, and propose adding the missing config file instead of keeping the rewrite.
- **If the run stalls** (no new console output for several minutes, typically a `REPOSITORY_*` scanner stuck on network): stop the container, then read the reports anyway — `megalinter-reports/` is written incrementally, so completed linters are still usable. Return their results with `status: "failure"` and a `failure_reason` naming the linters that never finished.
...
Distinguish **blocking** linters (❌, fail the job) from non-blocking ones (⚠️, `DISABLE_ERRORS: true`). A `failure` status means the job/run itself broke (container engine, network, configuration): include a `"failure_reason"` field with a short cause excerpt instead of linter errors. In watch mode, a `"job_url"` field may be added for reference. In local `--fix` runs, add a `"fixed_files"` field listing the source files the fixers modified (after the churn audit above).
