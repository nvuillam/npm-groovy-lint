...
- **Custom flavor run**: when the caller provides a custom flavor image (repositories using https://megalinter.io/latest/custom-flavors/), pass it with `--image <registry/path:tag>`.
...
- If the run produces no output for several minutes (typically a `REPOSITORY_*` scanner stuck on network), kill the container, then digest the reports anyway: `megalinter-reports/` is written incrementally, so return the completed linters normally with `status: "failure"` and a `failure_reason` naming the linters that never finished.
...
- When run with `--fix`, add a `"fixed_files"` field listing the source files the fixers modified (`git status --short`, excluding `megalinter-reports/`), so the caller can audit the changes — flag it explicitly in your summary if a formatter rewrote a large share of the repository (likely a formatter-config mismatch the caller must review).
