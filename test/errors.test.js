...
describe("Errors", function () {
  it("(API:source) should trigger a parse options error", async function () {
    const prevFileContent = fs
      .readFileSync("./lib/example/SampleFile.groovy")
      .toString();
    ...
    assert(
      linter.status === 2,
      `Linter status is 2 (${linter.status} returned)`,
    );
    assert(
      linter.error.msg.includes("Invalid option"),
  ...
  it("(API:source) should trigger a loglevel failon consistency error", async function () {
    ...
    assert(
      linter.error.msg.includes("failon option"),
      "failon + loglevel options consistency error detected",
    );
  });
  ...
  it("(API:source) should trigger a codenarc error", async function () {
    ...
    assert(
      linter.error &&
        linter.error.msg.includes("java.io.FileNotFoundException"),
