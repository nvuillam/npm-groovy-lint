...
describe("Lint with API", () => {
  ...
  it("(API:file) should generate text console output and stats", async function () {
    ...
    assert(
      linter.outputString.includes("warning"),
      "Output string contains warning",
    );
    assert(
      linter.lintResult.summary.totalFoundWarningNumber > 0,
      "Warnings found",
    );
  ...
  it("(API:file) should generate text console output with loglevel=warning", async function () {
    ...
    assert(
      !linter.outputString.includes("info"),
      "Output string should not contain info",
    );
    assert(
      linter.lintResult.summary.totalFoundWarningNumber > 0,
      "Warnings found",
  ...
  it("(API:file) should generate json output with rules", async function () {
    ...
    assert(
      linter.outputString.includes(`"totalFoundWarningNumber":`),
      "Property totalFoundWarningNumber is in result",
    );
    assert(
      linter.lintResult.summary.totalFoundWarningNumber > 0,
      "Warnings found",
  ...
  it("(API:file) should generate codenarc HTML file report", async function () {
    ...
    assert(
      fs.existsSync(reportFileName),
      `CodeNarc HTML report generated at ${reportFileName}`,
    );
  ...
  it("(API:file) should generate codenarc XML file report", async function () {
    ...
    assert(
      fs.existsSync(reportFileName),
      `CodeNarc XML report generated at ${path.resolve(reportFileName)}`,
    );
    fs.removeSync(reportFileName);
    checkCodeNarcCallsCounter(1);
  });
  ...
  it("(API:file) should use --codenarcargs to generate XML report", async function () {
    ...
    assert(
      fs.existsSync(reportFileName),
      `XML HTML report generated at ${path.resolve(reportFileName)}`,
    );
    fs.removeSync(reportFileName);
    checkCodeNarcCallsCounter(1);
  });
  ...
  it("(API:files) should ignore fake_node_modules and groovy pattern", async function () {
    ...
    assert(
      linter.status === 1,
      `Expected linter status is 1 got ${linter.status}`,
    );
  ...
  it("(API:source) should run with source only (no parsing)", async function () {
    ...
    assert(
      linter.lintResult.files[0].errors.length > 0,
  ...
  it("(API:source) should run with source only (parse error)", async function () {
    ...
    assert(
      linter.lintResult.summary.totalFilesLinted === 1,
      `Expected 1 file linted got ${linter.lintResult.summary.totalFilesLinted}`,
    );
    assert(
      linter.lintResult.summary.totalFoundInfoNumber === 1,
      `Expected 1 info got ${linter.lintResult.summary.totalFoundInfoNumber}`,
    );
    assert(
      linter.lintResult.files[0].errors.length === 2,
      `Expected 2 errors got ${linter.lintResult.files[0].errors.length}`,
    );
    assert(
      linter.outputString.includes("NglParseError"),
      `Expected NglParseError got ${linter.outputString}`,
    );
    checkCodeNarcCallsCounter(1);
  });
  ...
  it("(API:file) should run on a list of files (absolute)", async function () {
    ...
    assert(
      Object.keys(linter.lintResult.files).length === 2,
      "Files array contains 2 files",
    );
    assert(
      linter.lintResult.summary.totalFoundErrorNumber === 4,
      "Error found",
    );
    assert(
      linter.lintResult.summary.totalFoundWarningNumber === 6,
      "Warnings found",
    );
    assert(
      linter.lintResult.summary.totalFoundInfoNumber === 65,
      "Infos found",
    );
    checkCodeNarcCallsCounter(1);
  });
  ...
  it("(API:file) should run on a directory", async function () {
    const linter = await new NpmGroovyLint(
      [process.execPath, "", "--verbose", EXAMPLE_DIRECTORY],
      {
        verbose: true,
      },
    ).run();
    ...
    assert(
      Object.keys(linter.lintResult.files).length === 12,
      `Expected 2 files got ${Object.keys(linter.lintResult.files).length}`,
    );
    assert(
      linter.lintResult.summary.totalFoundErrorNumber === 12,
      `Expected 12 errors to ${linter.lintResult.summary.totalFoundErrorNumber}`,
    );
    assert(
    ...
    assert(
      linter.lintResult.summary.totalFoundInfoNumber === 1649,
      `Expected 1649 infos to ${linter.lintResult.summary.totalFoundInfoNumber}`,
    );
    checkCodeNarcCallsCounter(1);
  });
});
