...
describe("Check range detection", function () {
  ...
  for (const ruleName of Object.keys(npmGroovyLintRules)) {
    ...
    if (
      npmGroovyLintRules[ruleName].rangeTests &&
      npmGroovyLintRules[ruleName]
    ) {
      if (
        npmGroovyLintRules[ruleName].rangeTests &&
        npmGroovyLintRules[ruleName].rangeTests.length > 0
      ) {
        // Process rule tests
        for (const testRule of npmGroovyLintRules[ruleName].rangeTests) {
          ...
          if (
            npmGroovyLintRules[ruleName].isCodeNarcRule == null ||
            npmGroovyLintRules[ruleName].isCodeNarcRule === true
          )
        ...
        it(`${ruleName} range tests has not been defined: please do it !`, async function () {
          assert(
            0 === 1,
            `${ruleName} range tests has not been defined: please do it !`,
          );
        });
      }
    }
