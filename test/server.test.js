...
describe.skip("Server", function () {
  it("(API:Server) should kill running server", async function () {
    ...
    assert(
      oldLinter.status === 0,
      `Linter status is 0 (${oldLinter.status} returned)`,
    );

    const linter = await new NpmGroovyLint(
      [process.execPath, "", "--killserver", "--no-insight", "--verbose"],
      {
        verbose: true,
      },
    ).run();
    ...
    assert(
      linter.outputString.includes("CodeNarcServer terminated"),
      "CodeNarcServer has been terminated",
    );
  ...
  it("(API:Server) should not succeed to kill running server", async function () {
    ...
    assert(
      linter.outputString.includes("CodeNarcServer was not running"),
      "CodeNarcServer not killed because not running",
    );
