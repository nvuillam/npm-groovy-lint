...
describe("Lint & fix with API", function () {
  ...
  it("(API:source) should lint then fix only a list of errors", async function () {
    ...
    let errIdList = linter.lintResult.files[0].errors
      .filter((error) => error.fixable === true)
      .map((err) => err.id);
    ...
    assert(
      linter.lintResult.summary.totalFixedNumber >= 100,
      "Errors have been fixed",
    ); // can be more than the five sent errors, as there are other triggered fixes
    assert(
  ...
  it("(API:file) should lint and fix a Jenkinsfile in one shot", async function () {
    ...
    try {
      const prevFileContent = fs
        .readFileSync(tmpDir + "/Jenkinsfile")
        .toString();
      ...
      assert(
        linter.lintResult.summary.totalFixedNumber > 0,
        "Error have been fixed",
      );
      assert(
        linter.lintResult.files[Object.keys(linter.lintResult.files)[0]]
          .updatedSource !== prevFileContent,
        "File content has been updated",
      );
      assert(
        fs.existsSync("npm-groovy-fix-log.json"),
        "Output json file has been produced",
      );
      assert(
        !linter.outputString.includes("NaN"),
        "Results does not contain NaN",
  ...
  it("(API:file) should fix only some errors", async function () {
    ...
    try {
      ...
      assert(
        fs.existsSync("npm-groovy-fix-log-should-fix-only-some-errors.txt"),
  ...
  it("(API:file) should fix all errors except excluded rules", async function () {
    ...
    try {
      ...
      assert(
        fs.existsSync("npm-groovy-fix-log-should-exclude-some-rules.txt"),
        "Output txt file produced",
      );
      assert(
        !linter.outputString.includes("NaN"),
      ...
      const fixedRules = linter.lintResult.files
        ? Object.values(linter.lintResult.files).flatMap(
            (file) =>
              file.errors?.filter((e) => e.fixed).map((e) => e.rule) || [],
          )
        : [];
      const hasExcludedRule = fixedRules.some((rule) =>
        excludedRules.includes(rule),
      );
  ...
  it("(API:file) should fix groovy files", async function () {
    ...
    try {
      ...
      assert(
        fs.existsSync("npm-groovy-fix-log-should-fix-groovy-files.txt"),
        "Output txt file produced",
      );
      assert(
        !linter.outputString.includes("NaN"),
