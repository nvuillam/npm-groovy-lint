...
const SAMPLE_FILE_PARSE_ERROR_PATH =
  EXAMPLE_DIRECTORY + SAMPLE_FILE_PARSE_ERROR;
const SAMPLE_FILE_WITH_SPACES = "file with spaces.groovy";
const SAMPLE_FILE_WITH_SPACES_PATH =
  EXAMPLE_DIRECTORY + SAMPLE_FILE_WITH_SPACES;
...
async function copyFilesInTmpDir() {
  ...
  const tmpDir =
    rootTmpDir + "/" + ("tmpGroovyLintTest_" + Math.random()).replace(".", "");
...
function assertLintedFiles(output, expectedCount) {
  const lintedResult = output.matchAll(
    /npm-groovy-lint results in .\[1m(\S+).\[22m linted files/g,
  );
