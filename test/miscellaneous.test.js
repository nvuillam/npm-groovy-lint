...
describe("Miscellaneous", function () {
  it("(API:source) returns config file using path", async function () {
    ...
    assert(
      path.resolve(filePath) ===
        path.resolve("./lib/example/.groovylintrc.json"),
      ".groovylintrc.json has been returned",
    );
  });
  ...
  it("(API:source) load config using specific file name", async function () {
    const customConfigFilePath = process.platform.includes("linux")
      ? "~/.groovylintrc-custom.json"
      : os.tmpdir() + "\\.groovylintrc-custom.json";
    await fs.ensureDir("~/", { mode: "0777" });
    await fs.copy(
      "./lib/example/.groovylintrc-custom.json",
  ...
  it("(API:source) load standard config using string key", async function () {
    ...
    assert(
      rules["UnusedVariable"] &&
        rules["UnusedVariable"]["ignoreVariableNames"] == "_",
  ...
  it("(API:source) load custom config using string key", async function () {
    ...
    assert(rules["NoDef"] == "off", "NoDef is off");
    assert(
      rules["VariableName"] && rules["VariableName"]["severity"] === "info",
      "VariableName is severity info",
    );
    assert(rules["CompileStatic"] == "off", "CompileStatic is off");
    assert(
      rules["CouldBeSwitchStatement"] == "off",
      "CouldBeSwitchStatement is off",
    );
    assert(rules["CouldBeElvis"] == "off", "CouldBeElvis is off");
    assert(
      linter.status === 1,
  ...
  it("(API:source) return indent length without linting", async function () {
    ...
    if (tmpConfigFilePath) {
      ...
      if (
        configUser.rules &&
        configUser.rules["Indentation"] &&
        configUser.rules["Indentation"]["spacesPerIndentLevel"]
      ) {
        indentLength = configUser.rules["Indentation"]["spacesPerIndentLevel"];
      }
    }
    assert(
      indentLength != null && indentLength > 0,
      "Indent length has been returned",
    );
  });
  ...
  it("(API:source) return rules", async function () {
    ...
    assert(
      linter.lintResult.rules["AssertWithinFinallyBlock"].docUrl != null,
      "Rules doc urls are returned ",
    );
  });
  ...
  it.skip("(API:source) should cancel current request", async function () {
    ...
    assert(
      [0, 9].includes(linter1.status),
      `Linter 1 status is 9 or 0 (returned ${linter1.status}`,
    );
    assert(
      [0, 9].includes(linter2.status),
      `Linter 2 status is 9 or 0 (returned ${linter2.status}`,
    );
    assert(
      [0, 9].includes(linter3.status),
      `Linter 3 status is 9 or 0 (returned ${linter3.status}`,
    );
    assert(
      [0, 9].includes(linter4.status),
      `Linter 4 status is 9 or 0 (returned ${linter4.status}`,
    );
    assert(
      [0, 9].includes(linter5.status),
      `Linter 5 status is 9 or 0 (returned ${linter5.status}`,
    );
    assert(
      [0, 9].includes(linter6.status),
      `Linter 6 status is 9 or 0 (returned ${linter6.status}`,
    );
    assert(
      [0].includes(linterLast.status),
      `LinterLast status = 0 (returned ${linterLast.status}`,
    );
  ...
  it("(API:source) override java executable", async function () {
    ...
    if (javaPath) {
      ...
      if (
        javaPath.includes("hostedtoolcache") ||
        javaPath.includes("/opt/java/openjdk/bin/java")
      ) {
        console.log(
          "Skip test because for some strange reason it provokes a timeout on CI Windows and openjdk servers",
        );
        return;
      }
  ...
  it("(API:help) should show npm-groovy-lint help", async function () {
    ...
    assert(
      linter.outputString.includes("--verbose"),
      "--verbose is found in output text",
    );
  });

  it("(API:help) should show npm-groovy-lint help option", async function () {
    const linter = await new NpmGroovyLint(
      [process.execPath, "", "-h", "source"],
      {},
    ).run();
    assert(
      linter.status === 0,
    ...
    assert(
      linter.outputString.includes("-s, --source"),
      "npm-groovy-lint Help is displayed",
    );
  });
  ...
  it("(API:help) should show npm-groovy-lint version", async function () {
    ...
    assert(
      linter.outputString.includes(`npm-groovy-lint version ${v}`),
      `Provides version ${v}\nReturned outputString:\n${linter.outputString}`,
    );
    assert(
      linter.outputString.includes(`CodeNarc version`),
      `Provides CodeNarc version\nReturned outputString:\n${linter.outputString}`,
    );
    assert(
      linter.outputString.includes(`Groovy version`),
      `Provides CodeNarc version\nReturned outputString:\n${linter.outputString}`,
    );
  });
  ...
  it("(API:help) should show codenarc help", async function () {
    ...
    const linter = await new NpmGroovyLint(
      [process.execPath, "", "--codenarcargs", "-help"],
      {},
    ).run();
    assert(
      linter.status === 0,
    ...
    assert(
      linter.codeNarcStdOut.includes(
        "where OPTIONS are zero or more command-line options",
      ),
      "CodeNarc help is displayed",
    );
